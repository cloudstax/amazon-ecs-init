...
if [ -d "${TOPWD}/.git" ]; then
    ...
    VERSION_FLAG="-X github.com/cloudstax/amazon-ecs-init/ecs-init/version.Version=${version}"
    GIT_HASH_FLAG="-X github.com/cloudstax/amazon-ecs-init/ecs-init/version.GitShortHash=${git_hash}"
    GIT_DIRTY_FLAG="-X github.com/cloudstax/amazon-ecs-init/ecs-init/version.GitDirty=${git_dirty}"
fi
